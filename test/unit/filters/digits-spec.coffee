describe "digits", ->
  ...
  it "should do nothing if a non-number is passed in", ->
  ...
  it "should do nothing if no digits are passed in", ->
    expect(filter(200)).toEqual 200


  it "should return a string once filtered", ->
    expect(filter(50, 2)).toEqual "50"


  it "should return a single digit if 0 is passed in", ->
    expect(filter(0, 5)).toEqual "0"


  it "should round the integer component if the decimal places are discarded", ->
    expect(filter(1234.456, 4)).toEqual "1234"
    expect(filter(2345.678, 4)).toEqual "2346"


  it "should return the desired number of digits", ->
  ...
  it "should never crop digits from the integer component", ->
