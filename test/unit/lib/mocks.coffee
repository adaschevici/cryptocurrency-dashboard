mockCurrentTime = ->
  time = 1
  spyOn(Date.prototype, 'getTime').andCallFake ->
    time++
    time



angular.module('app.mock.data', [])
...
angular.module('app.mock.apiInterface', ['app.mock.data'])

  .factory 'apiInterface', ($q, mockData) ->
...
angular.module('app.mock.dashboardService', [])
...
angular.module('app.mock.trackerService', ['app.mock.dashboardService'])

  .factory 'trackerService', ($q, dashboardService) ->
    ...
    class Tracker
      ...
      startPolling: ->
      stopPolling:  ->
      getErrorMessage: -> 'There was an error!'

    tracker = new Tracker

    loadTracker:  -> tracker
    loadTrackers: -> { anc: tracker }
